...
key_dates (
  id, event, window, status, budget_note
  -- "window" is free text (e.g. "Late Nov 2026") — most real dates are estimated ranges, not exact ISO dates
)
```
...
- **Supabase project:** `xgmznitqwcvjgeoyghor` (project name may differ in dashboard) is authoritative. There is also a `vmi-expense-tracker` project (ref `zbmcxwanuwrydwcditdl`) visible to the Supabase MCP org — **that is a different, unused project, do not touch it.**
- **DB access:** Supabase migrations require the `systemsbyabel.com` org to have collaborator access on `xgmznitqwcvjgeoyghor`. If `mcp__Supabase__list_projects` doesn't show it, migrations are blocked until Abel grants access via the Supabase dashboard.
...
- **Phase 1 status:** scaffold complete — Next.js 16 + Tailwind v4 + Supabase SSR auth (`proxy.ts` gate) + full design token system in `app/globals.css` + all 12 route shells + nav shell (`components/nav/AppShell.tsx`) + Quick Add FAB shell (`components/QuickAddFab.tsx`, real logic lands in Phase 3) + a fully-styled static Command Deck (`app/(app)/page.tsx`) built directly against `command-deck-mono.html`, including the Budget-vs-Actual and Life-to-Date panels not in the original written spec. Placeholder values throughout — Phase 2 (schema) and Phase 3 (live wiring) are next.
- **Deployment:** no Vercel action taken yet — deploy only at the very end (Phase 8), after confirming with Abel. A Vercel project named `final-comand-center` already exists and is presumably linked to this GitHub repo.
