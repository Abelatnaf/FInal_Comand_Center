...
type Obligation = { obligation_id: string; title: string };
type LastEntry = {
  amount_minor: number;
  direction: string;
...
export function AddForm({
  ...
  /** Recent descriptions, offered as autocomplete so repeat merchants are one tap. */
  recentNotes: string[];
  knownTags: string[];
  ...
  const [accountId, setAccountId] = useState<string>(lastAccountId ?? accounts[0]?.id ?? "");
  ...
  function handleRepeatLast() {
    ...
    // Only reuse the account if it's still live -- an archived or deleted
    // account would otherwise submit a dead id.
    const account = accounts.find((a) => a.id === lastEntry.account_id);
    setAccountId(account?.id ?? lastAccountId ?? accounts[0]?.id ?? "");
  }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      {showMore && (
        <div className="card">
          ...
          <div className="row">
            <label className="section-label block mb-1.5" htmlFor="tags">
              Tags
            </label>
            <input
              id="tags"
            ...
            <p className="text-[13px] text-faint mt-1.5">
              Comma separated. Tags cut across categories — a trip, a client, a reimbursement.
            </p>
          </div>
          {obligations.length > 0 && direction === "out" && (
            <div className="row">
          ...
          {direction === "out" && (
            <div className="row">
              ...
              <p className="text-[13px] text-faint mt-1.5">
                Flags it for the year-end tax summary. Not tax advice — check with a professional.
              </p>
            </div>
          )}
          <div className="row">
      ...
      <input type="hidden" name="is_tax_deductible" value={taxDeductible ? "true" : "false"} />
