  ...
  return (
    <Glass className="overflow-x-auto">
      <table className="w-full text-sm min-w-[820px]">
        ...
        <tbody className="num">
          {rows.map((r) => (
            <tr key={r.cadet_week} className="border-t border-white/[0.05]">
              <td className="py-2 px-2 text-text-dim">{r.cadet_week}</td>
              <td className="py-2 px-2 text-text-faint text-xs whitespace-nowrap">{r.week_start}</td>
              <td className="py-2 px-2 text-text-faint text-xs whitespace-nowrap">{r.week_end}</td>
              <td className="py-2 px-2 text-right">{fmtUsd(r.total_expenses)}</td>
              <td className="py-2 px-2 text-right text-text-faint">{fmtUsd(r.necessary)}</td>
              <td className="py-2 px-2 text-right text-text-faint">{fmtUsd(r.discretionary)}</td>
              <td className="py-2 px-2 text-right">{fmtUsd(r.total_income)}</td>
              <td className="py-2 px-2 text-right">{fmtUsd(r.net)}</td>
              <td className="py-2 px-2 text-right text-silver font-medium">{fmtUsd(r.running_balance)}</td>
            </tr>
          ))}
        </tbody>
      </table>
