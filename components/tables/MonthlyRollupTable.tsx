      ...
      <table className="w-full text-sm min-w-[1400px]">
        ...
        <tbody className="num">
          {categoryData.map((row) => {
            ...
            return (
              <tr key={row.month} className="border-t border-white/[0.05]">
                <td className="py-2 px-2 text-text-dim whitespace-nowrap sticky left-0 bg-[#0d0d10]">
                  {fmtMonth(row.month)}
                </td>
                {categoryNames.map((c) => (
                ...
                <td className="py-2 px-2 text-right text-silver font-medium">{fmtUsd(balance?.running_balance ?? 0)}</td>
              </tr>
            );
          })}
        </tbody>
