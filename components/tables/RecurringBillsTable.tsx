  ...
  return (
    <form action={onSubmit} className="flex flex-wrap gap-2 items-end">
      ...
      <label className="flex items-center gap-1.5 text-xs text-text-dim pb-2">
        <input name="active" type="checkbox" defaultChecked={defaults?.active ?? true} className="accent-silver" />
        Active
      </label>
        ...
        <table className="w-full text-sm min-w-[800px] mt-2">
          ...
          <tbody>
            {bills.length === 0 ? (
              ...
              bills.map((b) =>
                editingId === b.id ? (
                  <tr key={b.id} className="border-t border-white/[0.05] bg-white/[0.02]">
