-- Restoring a backup has one hard problem: set_transaction_computed_fields()
-- looks up the LATEST fx rate on INSERT, so replaying a historical ETB row
-- would silently reprice it at today's rate. That is precisely the class of
-- bug this app exists to prevent, and it has bitten this codebase twice
-- before (v1's fix_fx_rate_lock_on_edit, v2's entries_direct_currency_entry).
--
-- The fix is a narrow, role-gated escape hatch: the trigger accepts a
-- caller-supplied rate ONLY when the current role is the database owner. A
-- PostgREST client is always `anon`/`authenticated`/`service_role` and can
-- never become `postgres`, so an ordinary insert still cannot forge a rate --
-- the guarantee holds for every path a client can actually reach. Inside a
-- SECURITY DEFINER function owned by postgres, current_user IS postgres,
-- which is what lets restore_from_backup() below write history faithfully.
...
begin
  ...
  if tg_op = 'UPDATE' and old.currency = new.currency then
    -- Never reprice an existing row just because it was edited.
    new.fx_rate_etb_per_usd := old.fx_rate_etb_per_usd;
  elsif tg_op = 'INSERT'
        and current_user = 'postgres'
        and new.fx_rate_etb_per_usd is not null
        and new.fx_rate_etb_per_usd > 0 then
    -- Restore path only (see comment above): trust the historical rate exactly
    -- as it was frozen when the entry was originally written.
    null;
  else
