...
select
  (select starting_sofi + starting_ally + starting_cash from public.settings where id = 1)
...
-- 52-week span from matriculation, matching the real Weekly Rollup tab shape.
create or replace view public.weekly_rollup
with (security_invoker = true) as
with weeks as (
  select
    gs as cadet_week,
    (select matriculation_date from public.settings where id = 1) + ((gs - 1) * 7) as week_start,
    (select matriculation_date from public.settings where id = 1) + ((gs - 1) * 7) + 6 as week_end
...
inc as (
  select cadet_week, sum(amount_usd) as total_income
  from public.income
  group by cadet_week
)
select
  ...
  (select starting_sofi + starting_ally + starting_cash from public.settings where id = 1)
    + sum(coalesce(inc.total_income, 0) - coalesce(tx.total_expenses, 0))
        over (order by w.cadet_week rows between unbounded preceding and current row) as running_balance
from weeks w
left join tx on tx.cadet_week = w.cadet_week
left join inc on inc.cadet_week = w.cadet_week
order by w.cadet_week;

-- 12-month span from matriculation, matching the real Monthly Rollup tab shape.
create or replace view public.monthly_rollup
with (security_invoker = true) as
with months as (
  select
    (date_trunc('month', (select matriculation_date from public.settings where id = 1))
...
with base as (
  select
    ...
    n.cash_actual,
    n.notes,
    n.sofi_actual + n.ally_actual + n.cash_actual as total_actual,
    (select starting_sofi + starting_ally + starting_cash from public.settings where id = 1)
