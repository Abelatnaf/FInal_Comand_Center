...
export default async function InsightsPage() {
  ...
  const [
    ...
    supabase.from("settings").select("matriculation_date").eq("id", 1).single(),
    ...
    supabase.from("weekly_rollup").select("cadet_week, total_expenses").order("total_expenses", { ascending: false }),
  ...
  const daysSinceMatriculation = matriculationDate
    ? Math.floor((now.getTime() - matriculationDate.getTime()) / 86400000)
    : 0;
  const daysElapsed = Math.max(1, daysSinceMatriculation);
  ...
  const weeksRemaining = lastSemester
    ? Math.max(0, (new Date(lastSemester.end_date).getTime() - now.getTime()) / (7 * 86400000))
    : 0;
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <StatCard label="Days Since Matriculation" value={String(daysSinceMatriculation)} size="small" />
        ...
        <StatCard
          ...
          value={topWeek ? `Cadet Wk ${topWeek.cadet_week}` : "—"}
        ...
        {lastSemester && (
          <StatCard
            label={`Weeks Remaining Until ${lastSemester.name} Ends`}
        ...
        {lastSemester && (
          <StatCard
            label={`Projected Balance — End of ${lastSemester.name}`}
