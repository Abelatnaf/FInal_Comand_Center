...
export default async function LedgerPage({ searchParams }: { searchParams: Promise<Filters> }) {
  ...
  let query = supabase
    .from("transactions")
    .select(
      "id, occurred_on, direction, amount_minor, currency, amount_usd_minor, category, note, account_id, payer_id, obligation_id, accounts(name), payers(label)"
    )
  ...
  const rows: TransactionRowData[] = (data ?? []).map((t) => ({
    id: t.id,
