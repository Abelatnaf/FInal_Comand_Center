...
export default async function AccountDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [balanceRes, accountsRes, transactionsRes, transfersRes] = await Promise.all([
    ...
    supabase
      ...
      .select("id, occurred_on, direction, amount_minor, category_name, category_icon, category_color, note")
      .eq("account_id", id)
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="card row flex items-center gap-3">
        ...
        <Amount minor={BigInt(account.balance_minor ?? 0)} className="text-[19px] text-text" />
      </div>
